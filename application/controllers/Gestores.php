...
class Gestores extends CI_Controller {
	...
	public $controller = "gestores";

	public function __construct()
	{
		parent::__construct();		
		$this->load->model('Gestores_model','',TRUE); 
		// Control Sessión
	...
	public function index($table_page=null,$id=null,$search=null)
	{
		...
		$data['table'] 			= $this->Gestores_model->table($table_page*$table_limit,$table_limit,$data['search']);
		$data['table_rows'] 	= $this->Gestores_model->table_rows($data['search']);
	...
	{          
		...
		$this->form_validation->set_rules('rad_dni', 'DNI', 'trim|required|is_unique[administradores.rad_dni]|callback_vdni',
				array('is_unique' => 'El DNI introducido ya está registrado')
			);
		$this->form_validation->set_rules('rad_apellidos', 'Apellidos', 'trim|required');
		$this->form_validation->set_rules('rad_nombres', 'Nombres', 'trim|required');
		# messages
	...
	{			
		...
		$data['row'] = $this->Gestores_model->read($rus_id);
	...
	public function delete($rus_id=false)
	{
        ...
        if($rus_id===FALSE)
		{
			...
			$check = $this->Gestores_model->delete($rus_id);
	...
	public function check_usuario()
  	{
      	return $this->Gestores_model->check_usuario();
  	}

  	public function check_dni()
  	{
      	return $this->Gestores_model->check_dni();
  	}

  	public function check_correo()
  	{
      	return $this->Gestores_model->check_correo();
  	}

  	public function vdni()
	{
	    return $this->Gestores_model->dni();
	}
